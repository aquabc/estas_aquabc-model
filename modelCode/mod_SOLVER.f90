...
contains
    ...
    subroutine SOLVE(PELAGIC_BOX_MODEL_DATA        , & 
        ...
        real(kind = DBL)          , intent(inout), &
            dimension(nkn, nstate)                :: SETTLING_VELOCITIES_OUTPUT

        real(kind = DBL), dimension(nkn, nstate), intent(inout) :: EFFECTIVE_DISSLOVED_FRACTIONS
        real(kind = DBL), dimension(nkn, nstate), intent(inout) :: EFFECTIVE_DEPOSITION_FRACTIONS
        real(kind = DBL), dimension(nkn, nstate), intent(inout) :: DEPOSITION_AREA_RATIOS
        
        
        real(kind = DBL), dimension(nkn) :: CHLA
        real(kind = DBL), dimension(nkn) :: SETTLING_VELOCITY_FACTORS
        real(kind = DBL), dimension(nkn, nstate) :: PRESCRIBED_SEDIMENT_FLUXES
        ...
        if (PELAGIC_SOLVER_NO == 1) then
            ...
            call CALC_DERIV &
                 (PELAGIC_BOX_MODEL_DATA, TIME             , TIME_STEP            , &
                  ...
                  EFFECTIVE_DISSLOVED_FRACTIONS , &
                  EFFECTIVE_DEPOSITION_FRACTIONS, &
                  DEPOSITION_AREA_RATIOS        , nkn, nstate)
            ...
            do i = 1, PELAGIC_BOX_MODEL_DATA % NUM_PELAGIC_BOXES
                ...
                do j = 1, PELAGIC_BOX_MODEL_DATA % NUM_PELAGIC_STATE_VARS
                    ...
                    if (PELAGIC_BOX_MODEL_DATA % PELAGIC_BOXES(i) % CONCENTRATIONS(j) < 0.0D0) then
                        PELAGIC_BOX_MODEL_DATA % PELAGIC_BOXES(i) % CONCENTRATIONS(j) = 0.0D0
    ...
    subroutine UPDATE_TIME_FUNCS &
        ...
        do i = 1, PELAGIC_BOX_MODEL_DATA % NUM_PELAGIC_BOXES
            BATHYMETRY_NO     = PELAGIC_BOX_MODEL_DATA % PELAGIC_BOXES(i) % BATHYMETRY_NO
            SURFACE_ELEVATION = PELAGIC_BOX_MODEL_DATA % PELAGIC_BOXES(i) % SURFACE_ELEVATION
            BOTTOM_ELEVATION  = PELAGIC_BOX_MODEL_DATA % PELAGIC_BOXES(i) % BOTTOM_ELEVATION
        ...
        do i = 1, PELAGIC_BOX_MODEL_DATA % NUM_PELAGIC_DISPERSIVE_LINKS
            ...
            MIXING_TS_NO = PELAGIC_BOX_MODEL_DATA % PELAGIC_DISPERSIVE_LINKS(i) % MIXING_TS_NO
                ...
                if ((SETTLING_VELOCITY_NO > 0).and.(SETTLING_VELOCITY_NO <= &
                    ...
                    SETTLING_VELOCITIES(i, j) = &
                        VALUE_FROM_TIME_SERIE(PELAGIC_BOX_MODEL_DATA % &
                                              SETTLING_VELOCITIES(SETTLING_VELOCITY_NO), TIME, 1)
                else     
    ...
    subroutine CALC_DERIV &
        ...
        real(kind = DBL), dimension(nkn, nstate), intent(inout) :: EFFECTIVE_DEPOSITION_FRACTIONS
        real(kind = DBL), dimension(nkn, nstate), intent(inout) :: DEPOSITION_AREA_RATIOS
        real(kind = DBL), dimension(nkn, nstate), intent(inout) :: PRESCRIBED_SEDIMENT_FLUXES
        
        include 'ECOLOGICAL_MODEL_INTERFACES.inc'
        ...
        real(kind = DBL), dimension(PELAGIC_BOX_MODEL_DATA % NUM_PELAGIC_STATE_VARS) :: CONCENTRATIONS
        real(kind = DBL), dimension(PELAGIC_BOX_MODEL_DATA % NUM_MODEL_CONSTANTS)    :: MODEL_CONSTANTS
        real(kind = DBL), dimension(PELAGIC_BOX_MODEL_DATA % NUM_PELAGIC_STATE_VARS) :: KINETIC_DERIVS
        ...
        real(kind = DBL), dimension(nstate) :: RESUSPENSION_CONCENTRATIONS
        real(kind = DBL) :: SHEAR_STRESS

        real(kind = DBL), dimension(nkn) :: CHLA
        real(kind = DBL), dimension(nkn) :: SETTLING_VELOCITY_FACTORS

        real(kind = DBL), dimension(nkn, nstate) :: SEDIMENT_FLUXES
        ! ---------------------------------------------------------------------------
        ...
        do i = 1, PELAGIC_BOX_MODEL_DATA % NUM_PELAGIC_DISPERSIVE_LINKS
            FIRST_BOX_NO     = PELAGIC_BOX_MODEL_DATA % PELAGIC_DISPERSIVE_LINKS(i) % FIRST_BOX_NO
            SECOND_BOX_NO    = PELAGIC_BOX_MODEL_DATA % PELAGIC_DISPERSIVE_LINKS(i) % SECOND_BOX_NO
            DISPERSION_COEFF = DISPERSION_COEFFS(i)
            INTERFACE_AREA   = INTERFACE_AREAS(i)
            MIXING_LENGTH    = PELAGIC_BOX_MODEL_DATA % PELAGIC_DISPERSIVE_LINKS(i) % MIXING_LENGTH
            ...
            do j = 1, PELAGIC_BOX_MODEL_DATA % NUM_PELAGIC_STATE_VARS
                if (PELAGIC_BOX_MODEL_DATA % DIFFUSION_ON(j) == 1) then
                    ...
                    if (FIRST_BOX_NO > 0) then 
                        FIRST_BOX_CONC = &
                            PELAGIC_BOX_MODEL_DATA % PELAGIC_BOXES(FIRST_BOX_NO) % CONCENTRATIONS(j)
                    else
                    ...
                    if (SECOND_BOX_NO > 0) then 
                        SECOND_BOX_CONC  = &
                            PELAGIC_BOX_MODEL_DATA % PELAGIC_BOXES(SECOND_BOX_NO) % CONCENTRATIONS(j)
                    else
                    ...
                    if (FIRST_BOX_NO > 0) then
                        ...
                        PELAGIC_BOX_MODEL_DATA % ECOL_DISPERSION_DERIVS (FIRST_BOX_NO, j, DERIV_NO) = &
                             PELAGIC_BOX_MODEL_DATA % ECOL_DISPERSION_DERIVS(FIRST_BOX_NO, j, DERIV_NO) + &
                    ...
                    if (SECOND_BOX_NO > 0) then
                        ...
                        PELAGIC_BOX_MODEL_DATA % ECOL_DISPERSION_DERIVS (SECOND_BOX_NO, j, DERIV_NO) = &
                             PELAGIC_BOX_MODEL_DATA % ECOL_DISPERSION_DERIVS(SECOND_BOX_NO, j, DERIV_NO) + &
                             (((DISPERSION_COEFF * 8.64D4 * INTERFACE_AREA) / MIXING_LENGTH) * &
                        ...
                        if (PELAGIC_BOX_MODEL_DATA % SETTLING_ON(j) == 1) then
                            if (PELAGIC_BOX_MODEL_DATA % PELAGIC_BOXES(i) % &
                                ...
                                EFFECTIVE_DISSLOVED_FRACTIONS(i, j) = &
                                    PELAGIC_BOX_MODEL_DATA % PELAGIC_BOXES(i) % SAVED_OUTPUTS(SAVED_OUTPUT_NO)
                            else
                                EFFECTIVE_DISSLOVED_FRACTIONS(i, j) = &
                                    PELAGIC_BOX_MODEL_DATA % PELAGIC_BOXES(i) % DISSOLVED_FRACTIONS(j)
                            end if
                        
                            if (PELAGIC_BOX_MODEL_DATA % PELAGIC_BOXES(i) % OVERLAYING_PELAGIC_BOX == 0) then
        ...
        call PELAGIC_KINETICS &
                (node_active                                            , &
                 ...
                 CALLED_BEFORE                                          , &
                 SURFACE_BOXES                                          , &
                 PELAGIC_BOX_MODEL_DATA % ZOOPLANKTON_OPTION            , &
                 PELAGIC_BOX_MODEL_DATA % ADVANCED_REDOX_SIMULATION     , & 
                 USER_ENTERED_frac_avail_DON                            , &
                 PELAGIC_BOX_MODEL_DATA % LIGHT_EXTINCTION_OPTION       , &
                 PELAGIC_BOX_MODEL_DATA % CYANO_BOUYANT_STATE_SIMULATION, &
                 PELAGIC_BOX_MODEL_DATA % CONSIDER_NON_OBLIGATORY_FIXERS, &
                 PELAGIC_BOX_MODEL_DATA % CONSIDER_NOSTOCALES)
